...
#better plotting 
df_long <- Boston %>%
  mutate(across(-crim, as.numeric)) %>%  # Convert all columns except 'crim' to numeric
...
# step aic removed population, US, education and urban
# final model Sales ~ CompPrice + Income + Advertising + Price + ShelveLoc + Age

# vi) partition train test
set.seed(123)
training.samples <- createDataPartition(Carseats$Sales, p = 0.7, list = FALSE)
train.data  <- Carseats[training.samples, ]
test.data <- Carseats[-training.samples, ]

#use only the significant values from previous step
model <- lm(Sales ~  CompPrice + Income + Advertising + Price + ShelveLoc + Age, data = train.data)
summary(model)
...
cv_model <- train(Sales ~ CompPrice + Income + Advertising + Price + ShelveLoc + Age, 
                  data = Carseats, method = "lm", 
                  trControl = control)
cv_model